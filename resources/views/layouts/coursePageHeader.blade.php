                                  ...
                                  <div class="flex flex-col h-full py-5 overflow-y-auto bg-white border-r border-neutral-100">
                                      <div class="px-5">
                                          <div class="fle hidde items-start justify-between pb-1 mb-5">
                                              {{-- <h2 class="text-base font-normal leading-6 text-gray-900" id="slide-over-title">Course Content</h2> --}}
                                              <h2 class="text-base font-semibold leading-6">{{$enrollment->batch->name}}</h2>
                                          ...
                                          <ul class="sortable-content-list ml-4" style="list-style-type: none;">
                                              ...
                                              <li data-id="{{ $c->id }}" class="hover:text-violet-600">
                                                  <a class="sortable-item mb-2 text-[15px] font-ligh text-neutral-700 flex gap-3 items-center {{ $c->id == $video->id ? 'text-orange-600 font-medium' : '' }}" 
                                                     href="{{ route('recordings', ['batchId' => $batchId, 'cId' => Crypt::encrypt($c->id)]) }}">
                                                      @if ($c->type == 2)
                                                      <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-video {{ $c->id == $video->id ? 'text-orange-600' : '' }}"><path d="m22 8-6 4 6 4V8Z"></path><rect width="14" height="12" x="2" y="6" rx="2" ry="2"></rect></svg>
                                                      @else
                                                      <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-file-text {{ $c->id == $video->id ? 'text-orange-600' : '' }}"><path d="M15 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V7Z"></path><path d="M14 2v4a2 2 0 0 0 2 2h4"></path><path d="M10 9H8"></path><path d="M16 13H8"></path><path d="M16 17H8"></path></svg>
                                                      @endif
                                                      {{ $c->title }}
                                                      @if(optional($c->userProgress(Auth::id()))->status == 1)
