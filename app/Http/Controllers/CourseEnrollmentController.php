...
class CourseEnrollmentController extends Controller
{
    ...
    public function checkout($id)
    {
        ...
        if (Auth::user()->id == $enrollment->userId && $enrollment->hasPaid == 0) {
            ...
            $order  = $api->order->create(array('amount' => $amountPayable, 'currency' => 'INR', 'notes' => array('Name' => Auth::user()->name, 'Email' => Auth::user()->email, 'UserId'=> Auth::user()->id, 'Phone' => Auth::user()->mobile, 'Course' => $batchId->name, 'StartDate' => $batchId->startDate, 'EndDate' => $batchId->endDate, 'CourseId' => $batchId->id, 'TopicId' => $batchId->topicId )));
